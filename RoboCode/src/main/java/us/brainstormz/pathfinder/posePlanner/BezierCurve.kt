...
class BezierCurve(val ctrlPoints: List<Point3D>)/*: Collection<Point3D> by listOf(*ctrlPoints.toTypedArray())*/ {

    fun calculatePoint(t: Double): Point3D {
        val n = ctrlPoints.size - 1
        val t = t.coerceIn(0.1, 1.0)

        return ctrlPoints.fold(Point3D()) { acc, it ->
    ...
    fun getPointOnSegment(curveIndex: Double, t: Double): Point3D {

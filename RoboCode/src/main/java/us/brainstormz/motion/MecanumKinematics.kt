...
//    from Kinematics
    ...
    fun calculateMotorFeedforward(
        ...
        vels.zip(accels)
            .map { (vel, accel) -> Kinematics.calculateMotorFeedforward(vel, accel, kV, kA, kStatic) }
    ...
    fun robotToWheelVelocities(
        ...
        return listOf(
            robotVel.x - lateralMultiplier * robotVel.y - k * robotVel.r,
            robotVel.x + lateralMultiplier * robotVel.y - k * robotVel.r,
            robotVel.x - lateralMultiplier * robotVel.y + k * robotVel.r,
            robotVel.x + lateralMultiplier * robotVel.y + k * robotVel.r
        )
    ...
    fun robotToWheelAccelerations(
        robotAccel: PositionAndRotation,
        trackWidth: Double,
        wheelBase: Double = trackWidth,
        lateralMultiplier: Double = 1.0
    ) =
